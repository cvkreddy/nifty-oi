...
def get_futures_symbol():
    ...
    for delta in [0, 1]:
        ...
        y = str(now.year)[2:] 
        if now.month + delta > 12: y = str(now.year + 1)[2:]
...
def compute_tf_signals(candles, label, st_period, st_multiplier):
    ...
    if len(closes) >= 9 and ema7 and ema15:
        pe7, pe15 = calc_ema(closes[:-1], 7), calc_ema(closes[:-1], 15)
        if pe7 and pe15:
...
def process_chain(raw):
    ...
    for item in raw:
        ...
        result[strike] = {
            "strike": strike,
            "call_oi": call_oi, "call_oi_chg": call_oi - prev.get("call_oi", call_oi) if prev else 0, "call_oi_chg_day": call_oi - base.get("call_oi", call_oi) if base else 0,
            "call_vol": call_vol, "call_vol_oi": round(call_vol/call_oi,2) if call_oi else 0,
            "call_iv": round(float(ce_gk.get("iv",0) or 0)*100,2), "call_ltp": call_ltp, 
            "call_ltp_chg": round(call_ltp - prev.get("call_ltp", call_ltp) if prev else 0, 2), "call_ltp_chg_day": round(call_ltp - base.get("call_ltp", call_ltp) if base else 0, 2),
            ...
            "put_oi": put_oi, "put_oi_chg": put_oi - prev.get("put_oi", put_oi) if prev else 0, "put_oi_chg_day": put_oi - base.get("put_oi", put_oi) if base else 0,
            "put_vol": put_vol, "put_vol_oi": round(put_vol/put_oi,2) if put_oi else 0,
            "put_iv": round(float(pe_gk.get("iv",0) or 0)*100,2), "put_ltp": put_ltp, 
            "put_ltp_chg": round(put_ltp - prev.get("put_ltp", put_ltp) if prev else 0, 2), "put_ltp_chg_day": round(put_ltp - base.get("put_ltp", put_ltp) if base else 0, 2),
...
def classify_strike_oi_flow(v, prev_spot, spot):
    pup = spot > prev_spot + 5 if prev_spot else False
    pdn = spot < prev_spot - 5 if prev_spot else False
    ...
    return {"condition": cf[0], "signal": cf[1], "emoji": cf[2]}, {"condition": pf[0], "signal": pf[1], "emoji": pf[2]}, {"net_note": "—"}
...
def refresh():
    ...
    try:
        ...
        # Max Pain
        min_loss, max_pain = float("inf"), sorted(chain.keys())[0] if chain else 0
        ...
        gex_data = [{"strike":s,"net_gex":v["call_gex"] - v["put_gex"]} for s,v in sorted(chain.items()) if abs(s-atm) <= 500]
        ...
        intelligence = {
            ...
            "gex_profile": gex_data[:11], "gex_flip": gex_flip,
            "alerts": alerts, 
            "index_technicals": {
